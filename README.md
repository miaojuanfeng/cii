...
直接在视图中输出字符串固然很方便，但无法判断视图中是否有字符串需要替换，如果在视图中没有字符串需要替换，在视图输出前依旧遍历视图字符串来查找要替换的字符串是很浪费时间的、没意义的，所以CII对此进行了性能优化，每次调用benchmark对象的方法，要替换字符串的次数就加1，调用方法多少次，就替换多少次。调用方法的次数为0，视图输出前不进行查找替换。
...
仅此一项优化，在视图没有字符串需要替换时，性能提高了7倍。需要替换一次字符串时，性能提高了1倍。这是一种空间换时间的做法，稍稍增加了空间复杂度（需要内存记录替换次数，共12字节），大大降低了时间复杂度。
